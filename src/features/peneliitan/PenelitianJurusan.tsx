...
import { jurusanData, semesters, StatusPenelitian } from "../../mock/db";
import { useFetchData } from "../../hooks/useFetchData";
import { getPenelitianByJurusan } from "../../mock/authService";
import { StateDisplay } from "../../components/ui/StateDisplay";

export const STATUS_COLOR: Record<StatusPenelitian, string> = {
...
export const STATUS_LABEL: Record<StatusPenelitian, string> = {
  accepted: "Disetujui",
  rejected: "Ditolak",
  pending: "Menunggu",
...
export default function PenelitianJurusan() {
    const navigate = useNavigate();
    const user = useAuthStore((state) => state.user);
    const isTataUsaha = user?.jabatan === "tata-usaha";
    const isKarodi = user?.jabatan === "kaprodi";
    ...
    const { data, loading, error } = useFetchData({
        fetchFn: () => getPenelitianByJurusan(jurusanIdNum, selectedSemesterId, search),
        dependencies: [jurusanIdNum, selectedSemesterId, search]
    });

    return (
        <DashboardLayout>
            <Breadcrumb items={[{ label: "Penelitian" }, {label: "Jurusan", onClick:() => navigate("/tri-dharma/penelitian/jurusan")}, { label: "Semester", isActive: true }, ]}></Breadcrumb>
            <div className="bg-dark-navy flex flex-col lg:flex-row justify-between rounded-2xl w-full px-5 lg:px-10 py-6 mb-8">
                <div>
                    ...
                    <p className="text-light-blue font-medium text-[15px]">Lihat daftar penelitian</p>
            ...
            <div className='flex flex-col md:flex-row gap-3 items-center  mb-3'>
                <input type="text" placeholder="Cari penelitian..." value={search} onChange={(e) => setSearch(e.target.value)} className='border bg-white border-gray-300 rounded px-4 h-11 w-full  focus:outline-none focus:border-primary-gold' />
            </div>
            ...
            <div className="flex gap-2 flex-col">
                ...
                <StateDisplay type={"error"} message="Error"/> 
                    ...
                    <StateDisplay type={"empty"} message="Belum ada penelitian"/> 
                ) : ( 
                    data?.penelitian.map((p) => (
                        <div onClick={() => navigate(`/tri-dharma/penelitian/${jurusanId}/${p.id}`)} className="bg-white rounded-2xl border border-card-cream divide-y divide-card-cream" key={p.id}>
                            <div className="flex justify-between items-center px-6 py-4 cursor-pointer">
                                ...
                                <p className={`text-sm border rounded-full px-3 py-1 ${STATUS_COLOR[p.status]}`}>{STATUS_LABEL[p.status]}</p>
                            </div>
                        </div>
                    ))
                )}
