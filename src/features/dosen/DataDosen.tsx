...
export default function DataDosen() {
  ...
  const jurusanOptions = useMemo(() => {
    return Array.from(new Set(dosenList.map((d) => d.jurusan))).filter(Boolean);
  }, [dosenList]);

  const filteredDosen = useMemo(() => {
    return dosenList.filter((d) => {
      const matchesSearch = d.nama.toLowerCase().includes(search.toLowerCase());
      const matchesFilter = filter === "" ? true : d.jurusan === filter;
    ...
    <DashboardLayout>
      ...
      {!loading && !error && (
        <div className="grid gap-3 pt-5">
          ...
          {filteredDosen.map((d) => (
            <div onClick={() => { navigate(`/data-dosen/${d.id}`); toast.info("Scroll ke bawah untuk melihat Tri-dharma dosen!") }} key={d.id} className="flex items-center justify-between bg-white p-4 rounded shadow border-primary-cream cursor-pointer hover:bg-gray-50 transition">
              <div>
