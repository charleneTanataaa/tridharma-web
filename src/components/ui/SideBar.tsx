...
function SidebarNav({ user, pathname, onNavigate }: {
    ...
    const jabatan = user.jabatan as Role;
    const filtered = menuItems.filter((item) => item.allowedRoles.includes(jabatan));
        ...
        <nav className="flex-1 overflow-y-auto text-[18px]">
            {filtered.map((item) => {
                if (item.isSection && item.children) {
                    const visibleChildren = item.children.filter((c) => c.allowedRoles.includes(jabatan));
                            ...
                            {isOpen && visibleChildren.map((child) => {
                                ...
                                return (
                                    <Link key={child.path} to={child.path} onClick={onNavigate}
                                        className={`flex items-center gap-3 px-5 py-4 transition ${isActive ? "bg-medium-navy text-white" : "text-light-blue hover:bg-primary-gold hover:text-white"
                                            }`}
                                    >
                ...
                return (
                    <Link key={item.path} to={item.path} onClick={onNavigate}
                        className={`flex items-center gap-3 px-5 py-4 transition ${isActive ? "bg-medium-navy text-white" : "text-muted-text hover:bg-primary-gold hover:text-white"
                            }`}>
    ...
    return (
        <>
            ...
            <button
                ...
                className={`lg:hidden fixed top-4 left-4 z-40 bg-dark-navy text-white p-2 rounded ${isOpen ? "opacity-0 pointer-events-none" : "opacity-100"
                    }`}>
                <MdMenu size={24} />
            </button>
